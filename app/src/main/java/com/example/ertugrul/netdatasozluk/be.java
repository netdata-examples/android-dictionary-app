...
public class be extends Activity {
    ...
    String URL = "https://www.netdata.com/XML/751b4af0";
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if(item.getItemId() == R.id.action_ayarlar){
            Intent i = new Intent(be.this, geriBildirim.class);
            startActivity(i);
        }
        else if(item.getItemId() == R.id.action_ayarlar2){
            Intent i = new Intent(be.this, hakkinda.class);
            startActivity(i);
        }else{
    ...
    public void onCreate(final Bundle savedInstanceState) {
        ...
        if (vocabulary.bee != null) {
            for (int temp = 0; temp < vocabulary.bee.getLength(); temp++) {
                Node nNode = vocabulary.bee.item(temp);
                if (nNode.getNodeType() == Node.ELEMENT_NODE) {
                    Element eElement = (Element) nNode;
                    String a = getNode("dc_Adjective", eElement);
                    String b = getNode("dc_Comparative", eElement);
                    String c = getNode("dc_Superlative", eElement);
                    String d = getNode("dc_Adjective_type", eElement);
                    String e = getNode("dc_Example_Sentence", eElement);
                    arrList.add(a);
                    arrList2.add(b);
                    id.add(c);
                    arrList3.add(d);
        ...
        lv.setOnItemClickListener(new AdapterView.OnItemClickListener() {
            ...
            public void onItemClick(AdapterView<?> adapterView, View view, int i, long l) {
                ...
                AlertDialog.Builder alertDialog = new AlertDialog.Builder(be.this);
                for(int j=0;j<vocabulary.bee.getLength();j++){
                    if(arrList.get(j).equals(a)){
                        alertDialog.setTitle(arrList4.get(j));
                        String b = arrList2.get(j);
                        String c = id.get(j);
                        String d = arrList3.get(j);
                        alertDialog.setMessage("Comp : " + b +"\n"+ "Sup : " + c +"\n"+ "Adj : " + d);
                    }
                }
        ...
        inputSearch.addTextChangedListener(new TextWatcher() {
            ...
            public void onTextChanged(CharSequence cs, int arg1, int arg2, int arg3) {
                be.this.adapter.getFilter().filter(cs);
            }
    ...
    private class DownloadXML extends AsyncTask<String, Void, Void> {
        ...
        protected void onPreExecute() {
            super.onPreExecute();
            pDialog = new ProgressDialog(be.this);
